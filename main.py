import RLFA
from csp_dom_wdeg import CSP as csp
from csp_dom_wdeg import backtracking_search, forward_checking, mac, min_conflicts, dom_wdeg
import time
...
if __name__ == '__main__':
    ...
    print("FC")
    start = time.time()
    fc_result, constraint_checks, visited_nodes = backtracking_search(data, select_unassigned_variable=dom_wdeg, inference=forward_checking)
    end = time.time()
    print(fc_result)
    ...
    print("\nMAC")
    start = time.time()
    mac_result, constraint_checks, visited_nodes = backtracking_search(data, select_unassigned_variable=dom_wdeg, inference=mac)
    end = time.time()
    print(mac_result)
    print("Time elapsed: %.5f" % (end - start), "seconds")
    print("Constraint checks:", constraint_checks)
    ...
    print("\nMin Conflicts")
    start = time.time()
    minConflicts_result, constraint_checks, visited_nodes = min_conflicts(data)
    end = time.time()
